...
生成Java热图主要是为了全面浏览程序行为和发现瓶颈。主要步骤如下：

1. 获取线程转储:
   `jstack`是分析Java进程CPU使用情况的工具。运行`jstack`以获取线程转储。例如，如果Java应用程序的PID是1234，您可以使用以下命令：`jstack 1234 > dump.txt`
   
2. 将线程转储转换为火炬图（Flame Graph）:
   为此，您需要一个名为FlameGraph的工具，它可以从`github`上获得。获取它并安装所需的依赖项后，您可以使用提供的`stackcollapse-jstack.pl`脚本将Java线程转储转化为火炬图消耗台。
    `./stackcollapse-jstack.pl dump.txt > collapsed.txt`
    
3. 生成火炬图:
   FlameGraph工具也提供了一个叫作 `flamegraph.pl`的脚本来生成其最后的.gz形式的火炬图。
    `./flamegraph.pl collapsed.txt > flamegraph.svg`
    
4. 分析运行较慢的Spark函数：
   打开SVG火炬图并用浏览器查看。函数图的宽度代表了CPU时间的占比，越宽代表这个函数消耗的CPU时间越多。寻找到占比较大的部分即可找出运行较慢的函数。

## 分析Leopard火炬图示例
- 手动操作：https://blog.csdn.net/merrily01/article/details/107548208
- github spark-submit集成：https://github.com/spektom/spark-flamegraph

- jps获取Worker的进程id
```bash
jps
```
- 获取线程转储 [[为什么jstack spark worker报错：target process not responding or HotSpot VM not loaded]]
```bash
jstack 43 > dump.txt
```
- 将线程转储为火炬图
...
# 定义我们的Pod名字和Kubernetes命名空间
POD_NAME=leopard-worker-leopard9-55995c7b9f-blbxg
NAMESPACE=default   # 根据您的环境变量替换成正确的命名空间

# 创建一个.tar文件包含所有 .svg 文件在Pod的 /root 目录中
kubectl -n ${NAMESPACE} exec ${POD_NAME} -- bash -c 'tar -cvf /root/svgs.tar /root/*.svg'

# 将Pod的.tar文件复制到本地
kubectl -n ${NAMESPACE} cp ${POD_NAME}:/root/svgs.tar ./svgs.tar
...
### 脚本内容
#### stackcollapse-jstack脚本
stackcollapse-jstack.pl脚本内容如下：
```bash
#!/usr/bin/perl -w
#
